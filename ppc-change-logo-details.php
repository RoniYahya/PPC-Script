...
$template->loadTemplate("ppc-templates/ppc-change-logo-details.tpl.html");

$template->includePage("{TABS}","advertiser-loggedin-header.php");
$template->includePage("{FOOTER}","common-footer.php");

$form=new Form("manage","ppc-change-logo-details.php");
...
$perpagesize = 10;
$total=$mysql->echo_one("select count(*) from ad_logos_details  where cid='$uid' and user_status='2'".$stat);

$p=$paging->page($total,$perpagesize,"","ppc-change-logo-details.php?status=$st");
...
$template->openLoop("LOGO","SELECT a.id, a.name, a.ad_logos_name, a.status FROM ad_logos_details a  where cid='$uid' and user_status='2' " .$stat."order by id DESC LIMIT ".(($pageno-1)*$perpagesize).", ".$perpagesize);
