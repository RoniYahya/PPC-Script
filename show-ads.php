...
include("locale/messages.".$lang_code.".inc.php");
if(!$adunitrow)
{
	exit(0);
}
...
//{
//	$commonid=$mysql->echo_one("select common_account_id from ppc_publishers where uid='$adunitrow[47]'");
//	$singleaccount.="AND x.common_account_id!='$commonid' AND x.parent_status!=0";
...
//$publisher=mysql_query("select status,traffic_analysis from ppc_publishers where uid='$adunitrow[47]'".$publisher_st);
//$publisher_row=mysql_fetch_row($publisher);
//
//
//$pub_status=$publisher_row[0];
//if($pub_status==0)
//{echo "select status,traffic_analysis from ppc_publishers where uid='$adunitrow[47]'".$publisher_st;
//	exit(0);
//}
...
//$restrictedsiteids=$adunitrow[52];
...
setcookie("verify_click", $verify_click, $expire);
	...
	$visit_query=mysql_query("select id from publisher_daily_visits_statistics_master where pid='$pid' and time='$visit_time' and page_url='$cur_page'");
		if($vid=mysql_fetch_row($visit_query))
			{
			$vid_row[0]=$vid[0];
			mysql_query("update publisher_daily_visits_statistics_master set direct_hits=direct_hits+$direct_status,referred_hits=referred_hits+$ref_status 
where  id='$vid[0]'");
...
else
{
	if($traffic_analysis==1)
	{
		...
		//echo "select id from publisher_daily_visits_statistics_master where pid='$pid' and time='$visit_time' and page_url='$cur_page' order by id desc limit 0,1";
...
AND c.status =1 
AND b.status =1 
AND (c.accountbalance >= b.maxcv OR c.bonusbalance >= b.maxcv) AND (c.accountbalance >0 OR c.bonusbalance >0) 
AND a.time_status=1 
".$time." 
".$adult_string." 
".$sqlstr." 
...
//echo mysql_num_rows($adresult)."hhh";
...
if($adunitrow['adlang']==0)  //anylanguages
{
	...
	{   
        ...
        $credittext='<img src="credit-image/'.$crstr.'/'.$ctimage.'" border="0" height="15px"/>';
...
if($adunitrow['adlang']==0)  //anylanguages
{
	if($clan=='en')
	{
		$credittext=$mysql->echo_one("select credit from ppc_publisher_credits where  id='".$adunitrow['ctext']."' ");
	}
	else
	{
	//echo "select credit from ppc_publisher_credits where parent_id=".$res['ctext']." and language_id='$lanid'";
		$credittext=$mysql->echo_one("select credit from ppc_publisher_credits where parent_id=".$adunitrow['ctext']." and language_id='$lanid'");
		if($credittext=='')
			$credittext=$mysql->echo_one("select credit from ppc_publisher_credits where  id='".$adunitrow['ctext']."' ");
	}
}
else
{
		...
		if($adunitrow['adlang']==$lanid )
		{
				...
				{
					 $credittext=$mysql->echo_one("select credit from ppc_publisher_credits where parent_id=".$adunitrow['ctext']." and language_id='".$adunitrow['adlang']."'");
				}
		}
		else
		{
...
.tb2, .tb3, .tb4 {background:<?php if($adunitrow['credit_text_positioning']==1 && $adunitrow['ctext'] !=0) echo $r1[2]; else echo $adunitrow['bg_color']; ?>; border-left:1px solid <?php echo $r1[2]; ?>; border-right:1px solid <?php echo $r1[2]; ?>;}
...
.bb2, .bb3, .bb4 {background:<?php if($adunitrow['credit_text_positioning']==0 && $adunitrow['ctext'] !=0) echo $r1[2]; else echo $adunitrow['bg_color']; ?>; border-left:1px solid <?php echo $r1[2]; ?>; border-right:1px solid <?php echo $r1[2]; ?>;}
...
if($adunittype==2)
{
		...
		?><table height="<?php if($adunitrow['ctext']==0 || $special_height_flag ==0) echo $adunitrow['height']; else if($adunitrow['ctext']!=0 && $special_height_flag ==1) echo $adunitrow['height']+15; ?>"  width="<?php  echo $adunitrow['width']; ?>" cellpadding="0" cellspacing="0" class="inout-table"><?php
		
		
		if($special_height_flag ==1)
		{
		if($adunitrow['credit_text_positioning']==1 && $credittext!="")                                                      //***Top Right Credit Text ***************    
			echo "<tr ><td class=\"inout-credit\"><a target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></td></tr>";
		}	
		...
		if($adrow=mysql_fetch_row($adresult))
		{
				...
				updateAgeingAndImpression ($mysql,$adunittype,$adrow,$ad_ageing_factor,$adunitid,$vid_row[0],$public_ip,$fraud_time_interval,$direct_status,$serverid);
		  ...
		  if($adrow[8] != "swf")
		  {
				...
				{
				?><div id="idiv_<?php echo $adrow[0]; ?>" class="inout_credit_over" onMouseOver="LoadCreditText(<?php echo $adrow[0]; ?>)">(i)</div><div class="inout_credit_data" id="crd_<?php echo $adrow[0]; ?>" style="display:none;" onMouseOut="DisableCreditText(<?php echo $adrow[0]; ?>)" ><table cellpadding="0" cellspacing="0"><tr ><td class="inout-credit"><a target="_blank"  href="<?php echo $server_dir.'index.php'; ?>"><?php echo $credittext;?></a></td></tr></table></div><?php
				}
				}
				?><a target="_blank" href="<?php  echo "{$server_dir}ppc-ad-click.php?id=$adrow[0]&kid=$adrow[4]&bid=$adunitid&vid=".$vid_row[0]."&vip={ENC_IP}&direct_status=".$send_direct_status;?>"><img  height="<?php echo $adunitrow['height']; ?>"  width="<?php  echo $adunitrow['width']; ?>"  src="<?php echo "$server_dir".$GLOBALS['banners_folder']."/$adrow[0]/$adrow[3]";?>" border="0"></a><?php
				if($special_height_flag !=1)
		        { 
		...
		{
				$alinkdemo=$server_dir."ppc-ad-click.php?Flsids=$adrow[0]&kid=$adrow[4]&bid=$adunitid&vid=".$vid_row[0]."&vip={ENC_IP}&direct_status=".$send_direct_status;
...
//	echo $strHardLinks;			
	...
	<script type="text/javascript">
		 ...
		 params.wmode="transparent";
	      ...
	      swfobject.embedSWF("<?php echo $server_dir.$GLOBALS['banners_folder'].'/'.$adrow[0].'/'.$adrow[3];?>", "myFlashDiv", "<?php  echo $adunitrow[1]; ?>", "<?php echo $adunitrow[2]; ?>", "9.0.0", "",flashvars,params,attributes);
</script>
...
<?php           if($special_height_flag !=1)
		...
		else
		{
				...
				$publicresults=mysql_query("select id, link, title, summary, displayurl,contenttype,hardcodelinks from ppc_public_service_ads where adtype=1 AND status=1 AND bannersize =".$adunitrow['max_size']." AND wapstatus =0 $ps_language order by lastacesstime ASC LIMIT 0,1");	
...
if($ini_error_status!=0)
{
	echo mysql_error();
				...
				?><a target="_blank" href="<?php  echo "{$server_dir}ppc-ps-ad-click.php?id=$rowpsad[0]";?>"><img height="<?php echo $adunitrow['height']; ?>"  width="<?php  echo $adunitrow['width']; ?>" src="<?php echo "$server_dir".$GLOBALS['admin_folder']."/".$GLOBALS['service_banners_folder']."/$rowpsad[0]/$rowpsad[3]";?>" border="0"></a><?php
				if($special_height_flag !=1)
		        { 
				if($adunitrow['credit_text_positioning']==0 && $credittext!="")                //***Bottom Right Credit Text ***************    
				{
				...
				$alinkdemo=$server_dir."ppc-ps-ad-click.php?id=$rowpsad[0]";
...
$strHardLinksPs.="flashvars.atar".($i+1)."='_blank';";
	...
	<script type="text/javascript">
		 ...
		 <?php  echo $strHardLinksPs;?>
	      ...
	      swfobject.embedSWF("<?php echo $server_dir.$GLOBALS['admin_folder'].'/'.$GLOBALS['service_banners_folder'].'/'.$rowpsad[0].'/'.$rowpsad[3];?>", "myFlashDivPub", "<?php  echo $adunitrow[1]; ?>", "<?php echo $adunitrow[2]; ?>", "9.0.0", "",flashvars,params,attributes);
</script>
...
$blockheight= $adunitrow['height'];
		...
		?>
		<table height="<?php  if($adunitrow['ctext']==0 || $special_height_flag ==0) echo $blockheight; else if($adunitrow['ctext']!=0 && $special_height_flag ==1) echo $blockheight+15; ?>"  width="<?php  echo  $adunitrow['width']; ?>" cellpadding="0" cellspacing="0" class="inout-table">
	...
	$currentcatalogadcount=0;
		if($adunitrow['orientaion']==2)//horizontal
			...
			//	if($adunitrow['credit_text_positioning']==1 && $adunitrow['ctext'] !=0)
			//		$catalogadblock.= "<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\"><a target=\"_blank\" href=\"$server_dir"."index.php\">$credittext</a></td></tr>";
					
					
					
			if($adunitrow['credit_text_positioning']==1 && $adunitrow['ctext'] !=0)   //left
			$catalogadblock.= "<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
					
					
				
			//if($adunitrow['credit_text_positioning']==1 && $adunitrow['ctext'] !=0 && $adunitrow['credit_text_alignment']==1)   //right
			//$catalogadblock.= "<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
...
if($adrow[9] != "swf")
{
							...
							$catalogadblock.='<a target="_blank" href="'.$server_dir.'ppc-ad-click.php?id='.$adrow[0].'&kid='.$adrow[4].'&bid='.$adunitid.'&vid='.$vid_row[0].'&vip='.$enc_ip.'&direct_status='.$send_direct_status.'" ><img  height="'.$catalog_height.'"  width="'.$catalog_width.'"  src="'.$server_dir.$GLOBALS['banners_folder'].'/'.$adrow[0].'/'.$adrow[2].'" border="0"></a> </div></td>';
...
params_<?php echo $adrow[0]; ?>.wmode="transparent";
	      ...
	      swfobject.embedSWF("<?php echo $server_dir.$GLOBALS['banners_folder'].'/'.$adrow[0].'/'.$adrow[2];?>", "myFlashDiv_<?php echo $adrow[0].$tmr; ?>", "<?php  echo $catalog_width; ?>", "<?php echo $catalog_height; ?>", "9.0.0", "",flashvars_<?php echo $adrow[0]; ?>,params_<?php echo $adrow[0]; ?>,attributes_<?php echo $adrow[0]; ?>);
</script>
	...
	$catalogadblock.='</div></td>';
						...
						$catalogadblock.='<td valign="top">';
						 ...
						 $catalogadblock.='<div class="inout-title"><a href="'.$server_dir.'ppc-ad-click.php?id='.$adrow[0].'&kid='.$adrow[4].'&bid='.$adunitid.'&vid='.$vid_row[0].'&vip='.$enc_ip.'&direct_status='.$send_direct_status.'"  target="_blank">'.$adrow[6].'</a></div>';
...
{							 
							  ...
							  $catalogadblock.='<a target="_blank" href="'.$server_dir.'ppc-ad-click.php?id='.$adrow[0].'&kid='.$adrow[4].'&bid='.$adunitid.'&vid='.$vid_row[0].'&vip='.$enc_ip.'&direct_status='.$send_direct_status.'" ><img  height="'.$catalog_height.'"  width="'.$catalog_width.'"  src="'.$server_dir.$GLOBALS['banners_folder'].'/'.$adrow[0].'/'.$adrow[2].'" border="0"></a> </div></td> </tr><tr><td valign="top">';
...
$catalogadblock.=';direction:rtl';
			...
			{				 
							 ...
							 $catalogadblock.='<a target="_blank" href="'.$server_dir.'ppc-ps-ad-click.php?id='.$rowpsad[0].'"><img  height="'.$catalog_height.'"  width="'.$catalog_width.'"  src="'.$server_dir.$GLOBALS['admin_folder'].'/'.$GLOBALS['service_banners_folder'].'/'.$rowpsad[0].'/'.$rowpsad[2].'" border="0"></a> </div></td>';
...
params_<?php echo $rowpsad[0]; ?>.wmode="transparent";
	      ...
	      swfobject.embedSWF("<?php echo $server_dir.$GLOBALS['admin_folder'].'/'.$GLOBALS['service_banners_folder'].'/'.$rowpsad[0].'/'.$rowpsad[2];?>", "myFlashDivPub_<?php echo $rowpsad[0].$tmrp; ?>", "<?php  echo $catalog_width; ?>", "<?php echo $catalog_height; ?>", "9.0.0", "",flashvars_<?php echo $rowpsad[0]; ?>,params_<?php echo $rowpsad[0]; ?>,attributes_<?php echo $rowpsad[0]; ?>);
</script>
	...
	$catalogadblock.='</div></td>';
				...
				$catalogadblock.='<div class="inout-title"><a href="'.$server_dir.'ppc-ps-ad-click.php?id='.$rowpsad[0].'" target="_blank">'.$rowpsad[4].'</a></div>';
...
{		
							  ...
							  $catalogadblock.='<a target="_blank" href="'.$server_dir.'ppc-ps-ad-click.php?id='.$rowpsad[0].'"><img  height="'.$catalog_height.'"  width="'.$catalog_width.'"  src="'.$server_dir.$GLOBALS['admin_folder'].'/'.$GLOBALS['service_banners_folder'].'/'.$rowpsad[0].'/'.$rowpsad[2].'" border="0"></a> </div></td></tr>	<tr> <td valign="top">';
...
params_<?php echo $rowpsad[0]; ?>.wmode="transparent";
	...
	$catalogadblock.='</div></td></tr>	<tr> <td valign="top">';
		  ...
		  $catalogadblock.='</tr>';	
				...
				//if($adunitrow['credit_text_positioning']==0 && $adunitrow['ctext'] !=0)
				//	$catalogadblock.= "<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\"><a target=\"_blank\" href=\"$server_dir"."index.php\">$credittext</a></td></tr>";
		
		
		    if($adunitrow['credit_text_positioning']==0 && $adunitrow['ctext'] !=0)   //left
			$catalogadblock.= "<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
					
					
				
			//if($adunitrow['credit_text_positioning']==0 && $adunitrow['ctext'] !=0 && $adunitrow['credit_text_alignment']==1)   //right
			//$catalogadblock.= "<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
		...
		else //vertical
			{
				//echo "here1";
			//	if($adunitrow['credit_text_positioning']==1 && $adunitrow['ctext'] !=0)
			//			$catalogadblock.= "<tr ><td class=\"inout-credit\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></td></tr>";
						
						
						
			if($adunitrow['credit_text_positioning']==1 && $adunitrow['ctext'] !=0)   //left
			$catalogadblock.= "<tr ><td class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
...
$singleflag=0;
	if($adunitrow['ad_type']==1 && $adunitrow['text_ad_type'] ==2)
		...
		?><table height="<?php if($adunitrow['ctext']==0 || $special_height_flag ==0)  echo $blockheight; else if($adunitrow['ctext']!=0 && $special_height_flag ==1) echo $blockheight+15; ?>"  width="<?php  echo $adunitrow['width']; ?>" cellpadding="<?php  //if($adunitrow['scroll_ad']==0) echo "5"; else echo "0";  ?>0" cellspacing="0" class="inout-table"><?php 
		$textadblock="";
		$textadblockstart="";
		$textadblockend="";
		$currentadcount=0;		
		if($adunitrow['orientaion']==2)//horizontal
			{
				if($adunitrow['credit_text_positioning']==1 && $credittext!="")   //left
					$textadblockstart.= "<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
					
					
				
			//	if($adunitrow['credit_text_positioning']==1 && $credittext!="" && $adunitrow['credit_text_alignment']==1)   //right
			//		$textadblockstart.= "<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
				...
				$textadblock.='<td class="inout-credit" ><a target="_blank" href="'.$server_dir.'index.php">'.$credittext.'</a></td>';
				...
				while($currentadcount < $adscount && $adrow=mysql_fetch_row($adresult))
				{
					...
					$textadblock.='<div class="inout-title"><a target="_blank"   href="'.$server_dir.'ppc-ad-click.php?id='.$adrow[0].'&kid='.$adrow[4].'&bid='.$adunitid.'&vid='.$vid_row[0].'&vip={ENC_IP}&direct_status='.$send_direct_status.'" >'.$adrow[2].'</a></div>';
						$textadblock.='<div class="inout-desc">'.$d_description.'</div>';
						$textadblock.='<div class="inout-url"><a  target="_blank"  href="'.$server_dir.'ppc-ad-click.php?id='.$adrow[0].'&kid='.$adrow[4].'&bid='.$adunitid.'&vid='.$vid_row[0].'&vip={ENC_IP}&direct_status='.$send_direct_status.'" >'.$ad_display_url.'</a></div>';
					 $textadblock.='</td>';
...
$dir_str='';
					if($rtl_lang_id==$rowpsad[5])
						$dir_str='style="direction:rtl"';
						$textadblock.='<td width="{WIDTH}" valign="top" '.$dir_str.'><div class="inout-title"><a target="_blank"   href="'.$server_dir.'ppc-ps-ad-click.php?id='.$rowpsad[0].'">'.$rowpsad[2].'</a></div>';
							$textadblock.='<div class="inout-desc">'.$d_description.'</div>';
							$textadblock.='<div class="inout-url"><a  target="_blank"  href="'.$server_dir.'ppc-ps-ad-click.php?id='.$rowpsad[0].'">'.$disurl.'</a></div>';
						$textadblock.='</td>';
						$currentadcount+=1;
					}
				}
				...
				/*if($adunitrow['credit_text_positioning']==0 && $credittext!="")
					$textadblockend.="<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></td></tr>";*/
					
				
					if($adunitrow['credit_text_positioning']==0 && $credittext!="")   //left
					$textadblockend.="<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
					
					
					//if($adunitrow['credit_text_positioning']==0 && $credittext!="" && $adunitrow['credit_text_alignment']==1)    //right
					//$textadblockend.="<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
					
					
					//<td class="inout-credit-next"  ></td>-->
			...
			//	if($adunitrow['credit_text_positioning']==1 && $credittext!="")
			//		$textadblockstart.="<tr ><td class=\"inout-credit\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></td></tr>";
					
					
					
		if($adunitrow['credit_text_positioning']==1 && $credittext!="")   //left
		$textadblockstart.= "<tr ><td  class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
		...
		//$textadblockstart.= "<tr ><td  class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
				...
				while($currentadcount < $adscount && $adrow=mysql_fetch_row($adresult))
				{
					...
					$textadblock.='<td ><div class="inout-title"><a target="_blank"   href="'.$server_dir.'ppc-ad-click.php?id='.$adrow[0].'&kid='.$adrow[4].'&bid='.$adunitid.'&vid='.$vid_row[0].'&vip={ENC_IP}&direct_status='.$send_direct_status.'" >'.$adrow[2].'</a></div>';
						$textadblock.='<div class="inout-desc">'.$d_description.'</div>';
						$textadblock.='<div class="inout-url"><a  target="_blank"  href="'.$server_dir.'ppc-ad-click.php?id='.$adrow[0].'&kid='.$adrow[4].'&bid='.$adunitid.'&vid='.$vid_row[0].'&vip={ENC_IP}&direct_status='.$send_direct_status.'" >'.$ad_display_url.'</a></div>';
					$textadblock.= '</td></tr>'; 
...
if($ini_error_status!=0)
{
	echo mysql_error();
					...
					if($rtl_lang_id==$rowpsad[5])
						$dir_str='style="direction:rtl"';
						$textadblock.='<tr height="{HEIGHT}"'.$dir_str.'>';
						$textadblock.='<td ><div class="inout-title"><a target="_blank"   href="'.$server_dir.'ppc-ps-ad-click.php?id='.$rowpsad[0].'">'.$rowpsad[2].'</a></div>';
							$textadblock.='<div class="inout-desc">'.$d_description.'</div>';
							$textadblock.='<div class="inout-url"><a  target="_blank"  href="'.$server_dir.'ppc-ps-ad-click.php?id='.$rowpsad[0].'">'.$disurl.'</a></div>';
						$textadblock.= '</td></tr>'; 
						$currentadcount=$currentadcount+1;
				...
				//{
				//	$textadblockend.="<tr ><td class=\"inout-credit\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></td></tr>";
				//}
				
				
				if($adunitrow['credit_text_positioning']==0 && $credittext!="")   //left
				$textadblockend.="<tr ><td class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
			...
			if($adunitrow['orientaion']==2)
				{
					...
					$textadblockstart=str_replace("{COLSPANNEW}",$currentadcount-1,$textadblockstart);
					$textadblockend=str_replace("{COLSPANNEW}",$currentadcount-1,$textadblockend);
...
?></div><div id='set_2' style='display:none;width:<?php  echo $adunitrow['width']; ?>' ></div><div id='set_3' style='display:none;width:<?php  echo $adunitrow['width']; ?>' ></div><div id='loading' style='display:none;width: 100%;height: 100%;text-align: center;vertical-align: middle;' ><img src="<?php echo $server_dir; ?>images/loaddata.gif"></div></body></html><?php
}
...
function updateAgeingAndImpression ($mysql,$adunittype,$row,$ad_ageing_factor,$adunitid,$vid,$public_ip,$fraud_time_interval,$direct_status,$serverid)
{
...
$lastAccTime=$currTime;
	...
	mysql_query("insert into publisher_impression_hourly_master(`id`,`pid`,`bid`,`time`,`cnt`,`server_id`) value('0','0','$adunitid','".$currTime."','1','$serverid')");	
...
var url="<?php echo $server_dir; ?>show-ads-paging.php?"+"set="+set+"&<?php echo $query_str; ?>";
//url=url+"&set="+set;
//url=url+"&adcount="+<?php //echo $adscount; ?>;
