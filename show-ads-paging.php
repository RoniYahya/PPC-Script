...
{
	$languages.="AND (a.adlang='$adlang' or adlang=0)";
	...
	$catalogadblock.='</div></td> </tr><tr><td valign="top">';
			 ...
			 $i++;
					...
					if($currentadcount<$catalog_adscount )
				{
					...
					$publicresults=mysql_query("select a.id, link, title, summary, displayurl,contenttype,hardcodelinks,adlang from ppc_public_service_ads a where  a.bannersize='".$adunitrow['catalog_size']."'  and adtype=2 AND status=1 AND wapstatus =0 $ps_language order by lastacesstime ASC  LIMIT 0,".($catalog_adscount-$currentadcount));
if($ini_error_status!=0)
{
	echo mysql_error();
}
			...
			//		$textadblockstart.= "<tr ><td  colspan=\"{COLSPAN}\" class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";
				...
				while($currentadcount < $adscount && $adrow=mysql_fetch_row($adresult))
				{
					...
					if($rtl_lang_id==$adrow[9])
						$rtlstr='style="direction:rtl;"';
						
						
					$textadblock.='<td  width="{WIDTH}" valign="top"'.$rtlstr.'>';
...
if($rtl_lang_id==$rowpsad[5])
						$rtlstr='style="direction:rtl;"';
						$textadblock.='<td width="{WIDTH}" valign="top"'.$rtlstr.'><div class="inout-title"><a target="_blank"   href="'.$server_dir.'ppc-ps-ad-click.php?id='.$rowpsad[0].'">'.$rowpsad[2].'</a></div>';
							$textadblock.='<div class="inout-desc">'.$d_description.'</div>';
		...
		//$textadblockstart.= "<tr ><td  class=\"inout-credit\">".$paginglinks."<div id=\"cred\"><a  target=\"_blank\"  href=\"$server_dir"."index.php\">$credittext</a></div></td></tr>";	
				...
				while($currentadcount < $adscount && $adrow=mysql_fetch_row($adresult))
				{
					...
					if($adunitrow['text_ad_type']!=2)
						...
						if($rtl_lang_id==$adrow[9])
						$rtlstr='style="direction:rtl;"';
					$textadblock.='<tr height="{HEIGHT}"'.$rtlstr.'>';
