...
$template->loadTemplate("publisher-templates/ppc-change-publisher-details.tpl.html");
...
$form=new Form("EditDetails","ppc-change-publisher-details-action.php");
...
$count=$mysql->echo_one("select country from ppc_publishers where uid=".$user->getUserID());
$ctrstr="";
...
$template->setValue("{EMAILFIELD}",$form->addTextBox("email",$mysql->echo_one("select email from ppc_publishers where uid=".$user->getUserID()),40,255));
...
$template->setValue("{SUBMIT}",$form->addSubmit($template->checkPubMsg(7004)));
$template->setValue("{FNAMEFIELD}",$form->addTextBox("firstname",$mysql->echo_one("select firstname from ppc_publishers where uid=".$user->getUserID()),40,255));
$template->setValue("{LNAMEFIELD}",$form->addTextBox("lastname",$mysql->echo_one("select lastname from ppc_publishers where uid=".$user->getUserID()),40,255));
$template->setValue("{PHNOFIELD}",$form->addTextBox("phone_no",$mysql->echo_one("select phone_no from ppc_publishers where uid=".$user->getUserID()),40,255));
$template->setValue("{ADDRESSFIELD}",$form->addTextArea("address",$mysql->echo_one("select address from ppc_publishers where uid=".$user->getUserID())));
$atax=$mysql->echo_one("select taxidentification from ppc_publishers where uid=".$user->getUserID());
