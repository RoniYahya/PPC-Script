...
{
	$res1=mysql_query("select pad.*,cad.* from wap_ad_block pad,ppc_custom_ad_block cad  where pad.id=cad.bid and cad.id='$id' and cad.wapstatus='1'");
...
mysql_query("update `ppc_custom_ad_block` set name='$adunit_name', title_color='$tc', desc_color='$dc', url_color='$gc', bg_color='$bc',credit_color='$credit_border_color', bordor_type='$border_type',credit_text='$credit_text',scroll_ad=$scroll_ad,adlang='$adlang' where id=$id and wapstatus='1';");
...
$inid=$mysql->echo_one("select bid from ppc_custom_ad_block where id=$id and wapstatus='1';");
$intype=$mysql->echo_one("select ad_type from wap_ad_block where id='$inid'");

if($intype!=7 && $intype!=6 )
{
header("location:ppc-admin-modify-wap-ad-unit.php?id=$id&ad_type_sticky=$ad_type_sticky&ad_pos=$sticky_ad_pos&users_resolution1=$users_resolution1&users_resolution2=$users_resolution2");
