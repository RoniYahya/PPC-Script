...
$customid=$_GET['id'];
phpsafe($customid);
$result=mysql_query("select * from ppc_ad_block where id='$customid'");
...
//$credittextpreview="<table class=\"inout-credit\" cellpadding=\"0\"  cellspacing=\"0\" ><tr ><td ><a target=\"_blank\" href=\"$server_dir"."index.php\">$ctext</a></td></tr></table>";		
...
if($row[3]==2 || $type=="image")
{
		...
		?><tr ><td style="background: url(../images/banner.gif) repeat;"><?php
		if($row['credit_text_positioning']==1 && $ctext!="")  
				{
				?>  
				<div id="idiv" class="inout_credit_over" onMouseOver="LoadCreditText()">(i)</div><div class="inout_credit_data" id="crd" style="display:none;" onMouseOut="DisableCreditText()" ><table cellpadding="0" cellspacing="0" class="inout-before-credit" ><tr ><td class="inout-credit"><a target="_blank"  href="<?php echo $server_dir.'index.php'; ?>"><?php echo $ctext;?></a></td></tr></table></div>
...
else if($row[3]==4 || $type=="catalog")
{
		...
		$catalog_width=$mysql->echo_one("select width from catalog_dimension where id='$row[42]'");
		$catalog_height=$mysql->echo_one("select height from catalog_dimension where id='$row[42]'");
...
//	echo"select width from catalog_dimension where id='$row[42]'";
		...
		$catalog_width=$mysql->echo_one("select width from catalog_dimension where id='$catalogid'");
		$catalog_height=$mysql->echo_one("select height from catalog_dimension where id='$catalogid'");
		...
		//	echo $row[43];
				...
				?><tr><td ><?php if($row[44]==1) 
							{ ?><table  cellpadding="5"  cellspacing="0" border=0  style="line-height: <?php echo $row[40]; ?>px;table-layout:fixed;overflow:hidden;<?php if($row[45]==1  && $i < $row[43]) { ?>border-bottom:solid 1px <?php echo $row[17]; ?>;<?php } ?>"><tr><td align="center"  style="width:<?php echo $catalog_width; ?>px"><div style=" height:<?php echo $catalog_height; ?>px;width:<?php echo $catalog_width; ?>px;background: url(../images/banner.gif) repeat; "></div></td><td align="left" valign="top"><span class="inout-title"><a href="#" target="_blank"><?php echo $ad_title; ?></a></span><br><span class="inout-desc"><?php echo $d_description; ?></span></td></tr></table><?php
				 } else 
				{ ?><table  cellpadding="5"  cellspacing="0" border=0 style="line-height: <?php echo $row[40]; ?>px;table-layout:fixed;overflow:hidden;<?php if($row[45]==1  && $i < $row[43]) { ?> border-bottom:solid 1px <?php echo $row[17]; ?>; <?php } ?>"><tr><td align="center"  style=" height:<?php echo $catalog_height; ?>px;"><div style="height:<?php echo $catalog_height; ?>px;width:<?php echo $catalog_width; ?>px;background: url(../images/banner.gif) repeat; "></div></td><td valign="top"><span class="inout-title"><a href="#" target="_blank"><?php echo $ad_title; ?></a></span><br><span class="inout-desc"><?php echo $d_description; ?></span></td></tr></table><?php 
				} ?></td></tr><?php
