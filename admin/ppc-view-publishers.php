...
//filtering country
//echo "select a.uid,a.username,b.name,a.domain,a.status from ppc_publishers a left join location_country b  on a.country=b.code where a.uid>0 $str_status  $str_country $com_account order by a.uid DESC";
$result=mysql_query("select a.uid,a.username,b.name,a.domain,a.status,a.common_account_id from ppc_publishers a left join location_country b  on a.country=b.code where a.uid>0 $str_status  $str_country $com_account order by a.uid DESC LIMIT ".(($pageno-1)*$perpagesize).", ".$perpagesize);
if(mysql_num_rows($result)==0 && $pageno>1)
	{
		$pageno--;
		$result=mysql_query("select  a.uid,a.username,b.name,a.domain,a.status,a.common_account_id  from ppc_publishers a,location_country b  where a.country=b.code $str_status  $str_country $com_account order by a.uid DESC LIMIT ".(($pageno-1)*$perpagesize).", ".$perpagesize);

	}
$total=$mysql->echo_one("select count(*) from ppc_publishers  a where a.uid>0  $str_status $str_country $com_account");
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
   <td   colspan="4" scope="row" class="heading">Manage Publishers</td>
  </tr>
</table>


<form name="ads" action="ppc-view-publishers.php" method="get">
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
 <tr>
    <td   height="50" colspan="">  Status <select name="status" id="status" >
...
<?php if($account_migration==1)
	  {
	  ?>	  Sigle-sign-on 
	  <select name="commonaccount">
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <?php } ?>  </td>
    <td width="50%" colspan="3" align="right"><?php echo $paging->page($total,$perpagesize,"","ppc-view-publishers.php?status=$status_value&country=$country_code"); ?></td>
  </tr>
</table>
  ...
  <tr class="headrow">
	...
	{ ?>
	<td width="23%"><strong>Single sign on</strong></td> <?php	
	}
    ?>
  </tr>
<?php
...
?>
  <tr <?php if(($i-1)%2==1) { ?>class="specialrow" <?php }?>>
    <td height="28" ><a href="view_profile_publishers.php?id=<?php echo $row[0]; ?>"><?php echo $row[1];?></a></td>
    <td ><?php 
	if($row[2]=="")
		echo "&nbsp;";
	else
	...
	if($row[5]==0)
	$str.= '&nbsp;|&nbsp;<a href="ppc-delete-publisher-action.php?id='.$row[0].'&url='.urlencode($url).'" onclick="return promptuser()">Delete</a>';
	}
   if($row[4]==-2)
	{
